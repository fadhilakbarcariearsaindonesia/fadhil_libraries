...
export function Switch({
  ...
  const thumbSyntax = resolveSyntax(composeSyntax(recipe?.slotSyntax?.thumb, slotSyntax?.thumb));
  ...
  return (
    <label
      ...
      data-slot={(props as Record<string, unknown>)['data-slot'] ?? 'switch'}
    >
      <input
        {...props}
        ...
        disabled={isDisabled}
      />
      <span className={styles.switchTrack} style={trackSyntax.style} aria-hidden="true" data-slot="switch-track">
        <span className={styles.switchThumb} style={thumbSyntax.style} data-slot="switch-thumb" />
      </span>
      <span className={styles.content}>
        <span className={styles.label} style={labelSyntax.style} data-slot="switch-label">{label}</span>
        {description ? (
          <span className={styles.description} style={descriptionSyntax.style} data-slot="switch-description">
            {description}
          </span>
        ) : null}
      </span>
